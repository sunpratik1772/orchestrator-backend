"""Iteration node — pass-through for now."""
...
def run(node: dict, ctx: RunContext, incoming: dict[str, Any]) -> dict[str, Any]:
    ...
    cap = int(cfg.get("maxIterations") or 1000)
    rows = _upstream_rows(incoming)[:cap]
    return {"rows": rows, "rowCount": len(rows), "iterations": len(rows)}
  
NODE_SPEC = _spec_from_yaml(_HERE / "loop.yaml", run)
